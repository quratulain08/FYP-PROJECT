...
const InternshipDetails: React.FC = () => {
  ...
  const handleTaskClick = (taskId: string) => {
    router.push(`/Industry/tasks/${taskId}`)
  }
  ...
  const openModal = async (student: Student) => {
    ...
    try {
      const response = await fetch(`/api/gradeReport/${slug}/${student._id}`)
      if (!response.ok) {
  ...
  const handleCheckboxChange = (studentId: string, isChecked: boolean) => {
    if (isChecked) {
      ...
      setTask((prevTask) => ({
        ...prevTask,
        assignedStudents: prevTask.assignedStudents.filter((id) => id !== studentId),
      }))
    }
  }
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full border border-gray-300 p-2 rounded-lg"
                    value={task.marks}
                    onChange={(e) => setTask({ ...task, marks: Number.parseInt(e.target.value) })}
                    required
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setTask({ ...task, weightage: Number.parseInt(e.target.value) })}
                    required
                  />
                </div>
              </div>
              ...
              <button
                ...
                className={`text-white px-4 py-2 rounded-lg transition duration-300 ${
                  internship?.isComplete ? "bg-gray-400 cursor-not-allowed" : "bg-green-600 hover:bg-green-700"
                }`}
