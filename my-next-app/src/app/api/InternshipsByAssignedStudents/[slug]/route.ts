...
// GET Method: Fetch Internship by Assigned Student ID
export async function GET(req: Request, { params }: { params: { slug: string } }) {
    try {
      const { slug: studentId } = params; // Dynamic parameter 'slug'
  
      // Ensure that 'studentId' is provided
      if (!studentId) {
      ...
      // Find the internship where 'assignedStudent' matches the provided 'studentId'
      const internship = await InternshipModel.findOne({ assignedStudents: studentId });
